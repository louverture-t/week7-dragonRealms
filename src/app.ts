...
class DragonApp {
  private readonly storage = new DragonStorageService();
  private readonly form = document.querySelector<HTMLFormElement>("#dragon-form");
  private readonly list = document.querySelector<HTMLDivElement>("#dragon-list");
  private readonly powerSlider = document.querySelector<HTMLInputElement>("#dragon-power");
  private readonly powerOutput = document.querySelector<HTMLOutputElement>("#dragon-power-output");
  private editingDragonId: string | null = null;
  ...
  private bindForm(): void {
    ...
    this.form.addEventListener("submit", (event) => {
      ...
      const type = formData.get("type") as keyof typeof DragonType | null;
  ...
  private validateForm(name: string, type: string | null, age: number, powerLevel: number, specialAbilities: string): string[] {
    const errors: string[] = [];
  ...
  private getFieldNameFromError(error: string): string {
    if (error.includes('Name')) return 'name';
    if (error.includes('type')) return 'type';
    if (error.includes('Age')) return 'age';
    if (error.includes('Power')) return 'power';
    if (error.includes('abilities')) return 'abilities';
    return 'name';
  }
  ...
  private updateFormButton(text: string): void {
  ...
  private editDragon(dragon: any): void {
    if (!this.form) return;

    // Populate form with dragon data
    const nameInput = this.form.querySelector<HTMLInputElement>('#dragon-name');
    const typeSelect = this.form.querySelector<HTMLSelectElement>('#dragon-type');
    const ageInput = this.form.querySelector<HTMLInputElement>('#dragon-age');
    const powerInput = this.form.querySelector<HTMLInputElement>('#dragon-power');
    const abilitiesTextarea = this.form.querySelector<HTMLTextAreaElement>('#dragon-abilities');

    if (nameInput) nameInput.value = dragon.name;
    if (typeSelect) typeSelect.value = dragon.type;
    if (ageInput) ageInput.value = dragon.age.toString();
    if (powerInput) {
      powerInput.value = dragon.powerLevel.toString();
      if (this.powerOutput) this.powerOutput.value = dragon.powerLevel.toString();
    }
    if (abilitiesTextarea) abilitiesTextarea.value = dragon.specialAbilities || '';
  ...
  private renderDragons(): void {
    ...
    const template = document.querySelector<HTMLTemplateElement>("#dragon-card-template");
    if (!template) {
    ...
    dragons.forEach((dragon) => {
      ...
      const card = fragment.querySelector<HTMLElement>('.dragon-card');
      if (!card) {
        return;
      }

      card.dataset.type = dragon.type;
      const nameEl = fragment.querySelector<HTMLElement>(".dragon-name");
      const metaEl = fragment.querySelector<HTMLElement>(".dragon-meta");
      const abilitiesEl = fragment.querySelector<HTMLElement>(".dragon-abilities");
      ...
      fragment.querySelectorAll<HTMLButtonElement>("button[data-action]").forEach((button) => {
